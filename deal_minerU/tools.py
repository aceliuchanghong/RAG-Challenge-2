...
def get_image_description_ollama(
    image_path: str,
    ...
    Args:
        ...
        prompt (str): 对模型下达的指令。
        model_name (str): 要使用的 Ollama 模型名称，默认为 qwen2.5vl。
        host (str): Ollama 服务地址，默认为本地 http://localhost:11434。
    ...
    if not os.path.exists(image_path):
        print(f"⚠️ 警告: 图片文件未找到，跳过: {image_path}")
        return None
    ...
    client = OpenAI(base_url=f"{host}/v1", api_key="no-key-required")

    print(f"🤖 正在向本地 Ollama 的模型 '{model_name}' 发送请求...")
...
def replacer(match: Match, file_stem: str, prompt: str) -> str:
    ...
    try:
        ...
        description = get_image_description_ollama(full_image_path, prompt)
...
if __name__ == "__main__":
    # image_path = "no_git_oic/upload_files_md_output/test_J/vlm/images/40cd422f3b1b5339c0a7ca0f6b06fe3b127d9bdc3819714b73d6dabea136418c.jpg"
    # print(f"{get_image_description_ollama(image_path)}")
    ...
    replacer_with_prompt = partial(replacer, prompt=prompt)
