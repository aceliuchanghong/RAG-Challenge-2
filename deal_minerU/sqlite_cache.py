...
def cache_to_sqlite_async(
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            # --- V V V --- START OF THE FIX --- V V V ---
            # 关键修复：在构建用于缓存的参数列表时，要排除 'self'
            params = kwargs.copy()
            if args:
                # 获取所有参数名
                ...
                # 如果是实例方法，第一个参数通常是 'self'，需要将其从缓存key的计算中排除
                if arg_names and arg_names[0] == "self":
                    arg_names.pop(0)  # 从名称列表中移除 'self'
                    actual_args.pop(0)  # 从值列表中移除 self 实例

                # 用剩下的参数构建字典
                params.update(dict(zip(arg_names, actual_args)))
            # --- ^ ^ ^ --- END OF THE FIX --- ^ ^ ^ ---
            ...
            try:
                if not re_run:
                    # 现在传递给 query_cache 的 params 不再包含 self 对象
                    cached_data, data_count = query_cache(
                        conn, function_name, params, debug=debug
                    )
                    if cached_data is not None:
                ...
                # 执行原始函数时，必须使用原始的 *args，因为它包含 self
                ...
                # 打印错误时，使用不含 self 的 params，避免日志过长或再次序列化失败
                error_msg = (
